...
def verify_skill(skill_name):
    ...
    question = SkillTest.query.get(question_ids[index])
...
def cleanup_duplicates():
    ...
    all_skills = Skill.query.order_by(Skill.user_id).all()
    seen, deleted = defaultdict(list), 0

    for skill in all_skills:
        key = (skill.user_id, skill.type, skill.name.strip().lower())
        seen[key].append(skill)
    ...
    db.session.commit()
    return f"Cleanup complete. Removed {deleted} duplicate skills."
...
def connect():
    ...
    if not Connection.query.filter_by(requester_id=requester_id, receiver_id=receiver_id).first():
        db.session.add(Connection(requester_id=requester_id, receiver_id=receiver_id, status='pending'))
        db.session.commit()
...
def respond_request():
    ...
    conn = Connection.query.get(request.form['conn_id'])
    if conn and conn.receiver_id == session['user_id']:
        conn.status = 'accepted' if request.form['action'] == 'accept' else 'rejected'
        db.session.commit()
...
def update_session():
    ...
    sess = Session.query.get(request.form.get('session_id'))
    if not sess or sess.teacher_id != session['user_id']:
        flash("Unauthorized or session not found.")
        return redirect(url_for('sessions'))

    sess.status = request.form.get('action')
    db.session.commit()
    flash(f"Session marked as {sess.status}.")
    return redirect(url_for('sessions'))
...
def join_session(session_id):
    ...
    if current_user.id not in [sess.teacher_id, sess.learner_id]:
...
@app.route('/chat_partial/<int:partner_id>', methods=['GET', 'POST'])
def chat_partial(partner_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    connected = Connection.query.filter(
    ...
    return render_template('partials/chat_content.html', user=user, partner=partner, messages=messages)
